...
def index(request):  # 首页页面
    return render(request, 'indexUI.html')  # 渲染页面
...
def customerIndex(request):  # 客户登陆过后显示的首界面
    ...
    return render(request, 'CustomerLoginedIndexUI.html')  # 渲染页面
...
def adminIndex(request):  # 管理员登陆过后的首界面
    ...
    return render(request, 'AdministratorLoginedIndexUI.html')  # 渲染页面
